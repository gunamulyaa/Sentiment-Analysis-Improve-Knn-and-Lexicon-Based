...
if ($cek_k == 0 ) {
	$sql = mysqli_query($koneksi, "INSERT INTO kvalues_baru VALUES(NULL,'$nilai_k','$n_positif','$n_negatif','$n_netral',now())") or die(mysqli_error($koneksi));
}else{
	$sql = mysqli_query($koneksi, "UPDATE kvalues_baru SET n_positif = '$n_positif', n_negatif = '$n_negatif', n_netral = '$n_netral', datetime_updated = now() WHERE nilai_k = '$nilai_k'") or die(mysqli_error($koneksi));
}

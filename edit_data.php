...
}else if ($_GET['type'] == 'kamus') {

	$id = $_POST['id'];
	$kata	= $_POST['kata'];
	$nilai = $_POST['code_sentiment'];
	$sql = mysqli_query($koneksi, "UPDATE kamus set kata = '$kata', code_sentiment = '$nilai' where id = '$id'") or die(mysqli_error($koneksi));

	if($sql){
		echo '<script>alert("Berhasil merubah data!"); document.location="index.php?halaman=kamus";</script>';
	}else{
		echo '<script>alert("Gagal merubah data!"); document.location="index.php?halaman=kamus";</script>';
	}
